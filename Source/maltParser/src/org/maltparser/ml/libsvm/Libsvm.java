...
/**
 * Implements an interface to the LIBSVM learner (currently the LIBSVM 2.91 is
 * used). More information about LIBSVM can be found at <a
 * href="http://www.csie.ntu.edu.tw/~cjlin/libsvm/" target="_blank">LIBSVM -- A
...
//		}
    ...
    /**
     * Reads an instance file into a svm_problem object according to the
     * Malt-SVM format, which is column fixed format (tab-separated).
     *
    ...
    public String toStringParameters(svm_parameter param) {
        ...
        sb.append("  SVM type      : ").append(svmtypes[param.svm_type]).append(" (").append(param.svm_type).append(")\n");
        sb.append("  Kernel        : ").append(kerneltypes[param.kernel_type]).append(" (").append(param.kernel_type).append(")\n");
        if (param.kernel_type == svm_parameter.POLY) {
            sb.append("  Degree        : ").append(param.degree).append("\n");
        }
        if (param.kernel_type == svm_parameter.POLY || param.kernel_type == svm_parameter.RBF || param.kernel_type == svm_parameter.SIGMOID) {
            sb.append("  Gamma         : ").append(dform.format(param.gamma)).append("\n");
            if (param.kernel_type == svm_parameter.POLY || param.kernel_type == svm_parameter.SIGMOID) {
                sb.append("  Coef0         : ").append(dform.format(param.coef0)).append("\n");
            }
        }
        if (param.svm_type == svm_parameter.NU_SVC || param.svm_type == svm_parameter.NU_SVR || param.svm_type == svm_parameter.ONE_CLASS) {
            sb.append("  Nu            : ").append(dform.format(param.nu)).append("\n");
        }
        sb.append("  Cache Size    : ").append(dform.format(param.cache_size)).append(" MB\n");
        if (param.svm_type == svm_parameter.C_SVC || param.svm_type == svm_parameter.NU_SVR || param.svm_type == svm_parameter.EPSILON_SVR) {
            sb.append("  C             : ").append(dform.format(param.C)).append("\n");
        }
        sb.append("  Eps           : ").append(dform.format(param.eps)).append("\n");
        if (param.svm_type == svm_parameter.EPSILON_SVR) {
            sb.append("  P             : ").append(dform.format(param.p)).append("\n");
        }
        sb.append("  Shrinking     : ").append(param.shrinking).append("\n");
        sb.append("  Probability   : ").append(param.probability).append("\n");
        if (param.svm_type == svm_parameter.C_SVC) {
            sb.append("  #Weight       : ").append(param.nr_weight).append("\n");
    ...
    /**
     * Parses the parameter string. The parameter string must contain parameter
     * and value pairs, which are separated by a blank or a underscore. The
     * parameter begins with a character '-' followed by a one-character flag
     * and the value must comply with the parameters data type. Some examples:
     *
     * -s 0 -t 1 -d 2 -g 0.4 -e 0.1 -s_0_-t_1_-d_2_-g_0.4_-e_0.1
     *
    ...
    /**
     * Converts the instance file (Malt's own SVM format) into the LIBSVM
     * (SVMLight) format. The input instance file is removed (replaced) by the
     * instance file in the LIBSVM (SVMLight) format. If a column contains -1,
     * the value will be removed in destination file.
     *
    ...
    public String toString() {
        final StringBuffer sb = new StringBuffer();
        sb.append("\nLIBSVM INTERFACE\n");
        sb.append("  LIBSVM version: ").append(LIBSVM_VERSION).append("\n");
        sb.append("  SVM-param string: ").append(paramString).append("\n");
