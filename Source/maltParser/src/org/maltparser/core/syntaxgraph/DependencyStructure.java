...
public interface DependencyStructure extends TokenStructure, SecEdgeStructure {
    ...
    /**
     * Returns the dependency node identified by <i>index</i> if it exists,
     * otherwise <i>null</i>.
     *
     * @param index the index of the dependency node
     * @return the dependency node identified by <i>index</i> if it exists,
     * otherwise <i>null</i>.
    ...
    /**
     * Adds an edge from the head to the dependent identified by the indices of
    ...
    /**
     * Replace the head of the dependent with a new head. The labels are not
     * affected.
     *
    ...
    /**
     * Remove an edge from the head to the dependent identified by the indices
     * of the dependency nodes.
     *
     * @param headIndex the index of the head dependency node
     * @param dependentIndex the index of the dependent dependency node
     * @throws MaltChainedException
     ...
     * identifies a dependency node. Index 0 should always be the root
     * dependency node and index s is the first terminal node and index n is the
     * last terminal node.
     *
    ...
    /**
     * Returns <i>true</i> if the head edge of the dependency node with
     * <i>index</i> is labeled, otherwise <i>false</i>.
     *
     * @param index the index of the dependency node
     * @return <i>true</i> if the head edge of the dependency node with
     * <i>index</i> is labeled, otherwise <i>false</i>.
     * @throws MaltChainedException
     */
    ...
    /**
     * Returns <i>true</i> if all nodes in the dependency structure are
     * connected, otherwise <i>false</i>.
     *
     * @return <i>true</i> if all nodes in the dependency structure are
     * connected, otherwise <i>false</i>.
     */
    public boolean isConnected();

    /**
     * Returns <i>true</i> if all edges in the dependency structure are
     * projective, otherwise <i>false</i>.
     *
     * @return <i>true</i> if all edges in the dependency structure are
     * projective, otherwise <i>false</i>.
     * @throws MaltChainedException
     */
    ...
    /**
     * Returns <i>true</i> if all dependency nodes have at most one incoming
     * edge, otherwise <i>false</i>.
     *
     * @return <i>true</i> if all dependency nodes have at most one incoming
     * edge, otherwise <i>false</i>.
     */
    public boolean isSingleHeaded();

    /**
     * Returns <i>true</i> if the dependency structure are a tree (isConnected()
     * && isSingleHeaded()), otherwise <i>false</i>.
     *
     * @return <i>true</i> if the dependency structure are a tree (isConnected()
     * && isSingleHeaded()), otherwise <i>false</i>.
     */
    ...
    /**
     * Sets the default edge label of the root according to the default root
     * label option
     *
     * @param rootLabelOption the default root label option
     * @param edgeSymbolTables a sorted map that maps the symbol table name to
